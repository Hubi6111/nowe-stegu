...
async def lifespan(app: FastAPI):
    logger.info("CV Engine starting — loading all models eagerly...")
    t0 = time.time()

    try:
        wall_prior.load_model()
        _warmup_status["wall_prior"] = True
        logger.info("  ✓ Wall prior (SegFormer-B5) loaded")
    ...
    loaded = sum(1 for v in _warmup_status.values() if v)
    logger.info("All models loaded: %d/%d in %.1fs", loaded, len(_warmup_status), time.time() - t0)
